...
def gaze_csv(path, offset=0.0):
    with open(path, "w", encoding="utf-8") as handle:
        handle.write(f"t,x,y\\n0,{0.1 + offset},0.2\\n1,{0.2 + offset},0.3\\n2,{0.3 + offset},0.4\\n")
...
with tempfile.TemporaryDirectory() as root:
    ...
    gaze_csv(os.path.join(root, "asd", "case_001.csv"), 0.00)
    gaze_csv(os.path.join(root, "unlabeled", "case_002.csv"), 0.01)
    gaze_csv(os.path.join(root, "ads", "case_003.csv"), 0.02)
    gaze_csv(os.path.join(root, "case_asd_004.csv"), 0.03)
    ...
    duplicate = os.path.join(root, "td")
    os.makedirs(duplicate)
    gaze_csv(os.path.join(duplicate, "copied_case.csv"), 0.00)
    try:
        modalities.get("eyegaze").extract_folder(root)
    except ValueError as error:
        check("duplicate raw recordings fail closed", "duplicate raw recording" in str(error))
    else:
        check("duplicate raw recordings fail closed", False)
