...
As of 2026-08-01 the engineering critical path is running ahead of schedule: the Aug 3–9 identity
...
| **2026-08-10** | Complete and sign one route-specific Institution Data Admission Pack per institution: exact data flow, data-user/controller/processor roles, purpose, legal basis/consent, DPIA/PIPIA, transfer mechanism, retention, onward access, audit/model recipients and incident contact | Legal/DPO review and institutional signatures; a generic MOU or node invitation is insufficient | Go/no-go checklist item; lawful processing and cross-border route |
| **2026-08-10** | Confirm ethics/IRB coverage for the exact pilot flow, learning mode, coordinator/test-set location, model publication and cross-border recipients | Ethics committee decision; a change from local supervised analysis can require amendment | Any run touching real participant recordings |
...
The admission-pack template, Hong Kong/深圳/北京/overseas route matrix, GBA restrictions and immediate
no-go conditions are in [`legal-and-policy.md`](legal-and-policy.md). In particular, `FED_COHORT=1`,
plaintext HTTP, a shared demo password, an unapproved centralized real test set, or unknown cloud/
backup/remote-admin locations cannot be accepted for a real-data pilot.
